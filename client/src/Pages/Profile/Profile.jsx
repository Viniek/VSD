...
function Profile() {
  ...
  const formik = useFormik({
    ...
    validate: function (values) {
      ...
      if (!values.maritual_status)
        errors.maritual_status = "Maritual staatus is required";
      if (!values.disability)
        errors.maritual_status = "maritual status is required";
  ...
  const formik1 = useFormik({
    ...
    validate: function (values) {
      ...
      if (values.oldPassword == values.newPassword)
        error.newPassword = "new password cannot be similar to old password ";
      if (!values.confirmPassword)
        error.confirmPassword = "Conirm passwordis required";
          ...
          <div className="profile_form_fields">
            ...
            <div className="profile_inputs">
              ...
              <select
                ...
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                value={formik.values.maritual_status}
              >
          ...
          <div className="profile_form_fields">
            ...
            <div className="profile_inputs">
              ...
              {formik.touched.next_of_kin && formik.errors.next_of_kin && (
                <p className="errors">{formik.errors.next_of_kin}</p>
              )}
            </div>
            ...
            <div className="profile_inputs">
              ...
              {formik.touched.next_of_kin_phone &&
                formik.errors.next_of_kin_phone && (
                  <p className="errors">{formik.errors.next_of_kin_phone}</p>
                )}
            </div>
          </div>
      ...
      <div className="ovellay" id="ovellay">
        <form action="" className="ovelay-form" onSubmit={formik1.handleSubmit}>
          <div className="close-btn-wrapper">
            <button className="close-btn" onClick={handleCloseOvellay}>
              X
            </button>
          </div>
          ...
          {formik1.touched.oldPassword && formik1.errors.oldPassword && (
            <p className="errors">{formik1.errors.oldPassword}</p>
          )}
          ...
          {formik1.touched.newPassword && formik1.errors.newPassword && (
            <p className="errors">{formik1.errors.newPassword}</p>
          )}
          ...
          {formik1.touched.confirmPassword &&
            formik1.errors.confirmPassword && (
              <p className="errors">{formik1.errors.confirmPassword}</p>
            )}
          <button className="change-password-submit-btn">submit</button>
        </form>
        {error && <p className="errors"> {error}</p>}
      </div>
      <button className="change-password-btn" onClick={handleOpenOvellay}>
        change password
      </button>
    </div>
  );
